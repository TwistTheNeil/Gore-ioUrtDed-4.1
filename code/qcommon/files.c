...
int FS_FOpenFileReadChooseSearchpath(qboolean fullSearchpath, const char *filename,	fileHandle_t *file, qboolean uniqueFILE) {
...
===============
*/
char **FS_ListFilteredFilesChooseSearchpath(qboolean fullSearchpath, const char *path, const char *extension, char *filter, int *numfiles) {
	searchpath_t	*chosenSearchpaths;
	int				nfiles;
	char			**listCopy;
...
static void FS_StartupChooseUnloadUnusedMaps(const char *gameName, qboolean unloadUnusedMaps, const char *mapName) {
	...
	if ((paths & 0x001) != 0) {
		FS_AddGameDirectoryChooseOnlyLoadRelevantPk3(fs_basepath->string, gameName, (lastPath == 0x001), (unloadUnusedMaps && (lastPath == 0x001)), mapName);
	}
	...
	if ((paths & 0x002) != 0) {
		FS_AddGameDirectoryChooseOnlyLoadRelevantPk3(fs_homepath->string, gameName, (lastPath == 0x002), (unloadUnusedMaps && (lastPath == 0x002)), mapName);
	}

	// check for additional base game so mods can be based upon other mods
	if ((paths & 0x010) != 0) {
		FS_AddGameDirectoryChooseOnlyLoadRelevantPk3(fs_basepath->string, fs_gamedirvar->string, (lastPath == 0x010), (unloadUnusedMaps && (lastPath == 0x010)), mapName);
	}
	
	if ((paths & 0x020) != 0) {
		FS_AddGameDirectoryChooseOnlyLoadRelevantPk3(fs_homepath->string, fs_gamedirvar->string, (lastPath == 0x020), (unloadUnusedMaps && (lastPath == 0x020)), mapName);
	}
