...
static void SV_Map_f( void ) {
	...
	if (!FS_CanLoadMapBsp(unloadUnusedMaps, (sv_pure->integer != 0), mapname, fixedMapname, sizeof(fixedMapname), errorMsg, sizeof(errorMsg))) {
...
static void SV_StartRecordAll(void) {
	...
	for (slot=0, client=svs.clients; slot < sv_maxclients->integer; slot++, client++) {
		...
		if (client->netchan.remoteAddress.type == NA_BOT || client->state != CS_ACTIVE || client->demo_recording) {
			continue;
		}
