...
void SV_DirectConnect( netadr_t from ) {
	...
	if ( !NET_IsLocalAddress (from) ) {
		...
		if (sv_limitConnectPacketsPerIP->integer > 0) {
			// We limit 12 connect packets from one IP address in the past 2 seconds.
			connect_t *connect = &svs.connects[0];
			int oldest = 0;
...
//			VM_Call( gvm, GAME_CLIENT_DISCONNECT, newcl - svs.clients );
	...
	if (newcl != NULL && // If server is full, give them the "server full" message below.
		...
		Com_DPrintf("Rejected a connection, too many clients from IP address %s\n", NET_AdrToString(from));
		return;
	}
...
static void SV_DisplayGotoHelp_f(client_t *cl) {
	SV_SendServerCommand(cl, "print \"    Available commands:\n\n\"");
	SV_SendServerCommand(cl, "print \"    \\help              - show available commands\n\"");
	...
	SV_SendServerCommand(cl, "print \"    \\allowgoto 1       - allow players to teleport to you\n\"");
	SV_SendServerCommand(cl, "print \"    \\allowgoto 0       - disallow players to teleport to you\n\"");
	SV_SendServerCommand(cl, "print \"    \\goto <client>     - teleport to a player%s\n\"", (sv_allowGoto->integer > 0) ? "" : " (currently disabled)");
	SV_SendServerCommand(cl, "print \"    \\regainstamina 1   - activate the regainstamina\n\"");
	SV_SendServerCommand(cl, "print \"    \\regainstamina 0   - deactivate the regainstamina\n\"");
	SV_SendServerCommand(cl, "print \"    \\playerlist        - display the list of online players\n\"");
}
...
static void SV_SavePosition_f(client_t *cl) {
	...
	FILE *pf;
	char *cl_guid, *sv_mapname;
	char filename[255];
	double x, y, z;

	if (!(sv_allowLoadPosition->integer > 0)) {
		SV_SendServerCommand(cl, "print \"Saving player position is disabled on server.\n\"");
		return;
	}
	...
	if (sv_gotoMsgBigtext->integer == 1) 	SV_SendServerCommand(cl, "cp \"%s\n\"", sv_saveSuccessMsg->string);
	else 									SV_SendServerCommand(cl, "print \"%s\n\"", sv_saveSuccessMsg->string);
	
	/* Saving the position permanently */
	
		/* Getting values */
		cl_guid = Info_ValueForKey(cl->userinfo, "cl_guid");
		sv_mapname = Cvar_VariableString("mapname");
		x = cl->savedPosition[0];
		y = cl->savedPosition[1];
		z = cl->savedPosition[2];
		
		/* Creating the directory which will contains all positions files (if it's not already created) */
		mkdir("savedpositions", S_IRWXU | S_IRGRP | S_IXGRP);
		
		/* Saving values to external file */
		sprintf(filename,"savedpositions/%s_%s",cl_guid,sv_mapname);	
		pf = fopen(filename,"w+");
		fprintf(pf,"%f\t%f\t%f",x,y,z);
		fclose(pf);
...
static void SV_LoadPosition_f(client_t *cl) {
	int	clId, loadPositionWaitTime, nextLoadPositionTime;
	playerState_t *myState;
	...
	if (!(cl->positionIsSaved)) {
		...
		if (pf != NULL) {
			/* Found a file containing player position */
			fscanf(pf,"%lf\t%lf\t%lf",&x,&y,&z);
			cl->savedPosition[0] = x;
			cl->savedPosition[1] = y;
			cl->savedPosition[2] = z;
			cl->positionIsSaved = qtrue;
		}
		else {
			/* Position has not been saved permanently */
	...
	myState->velocity[0] = 0;	/* Reset X speed*/
	myState->velocity[1] = 0;	/* Reset Y speed*/
	myState->velocity[2] = 0;	/* Reset Z speed*/
	
	cl->lastLoadPositionTime = svs.time;
	
	if (sv_gotoMsgBigtext->integer == 1) 	SV_SendServerCommand(cl, "cp \"%s\n\"", sv_loadSuccessMsg->string);
	else 									SV_SendServerCommand(cl, "print \"%s\n\"", sv_loadSuccessMsg->string);
}
...
/*
===========================
SV_UserAllowRegainStamina_f
===========================
*/
static void SV_UserAllowRegainStamina_f(client_t *cl) {

	if (Cmd_Argc() != 2) { return; }
	if (strlen(Cmd_Argv(1)) != 1) { return; }
	
	if (sv_regainStamina->integer == 0) {
		SV_SendServerCommand(cl, "print \"Regain stamina is disabled on this server\n\"");
		return;
	}
	
	if (Cmd_Argv(1)[0] == '0') {
		cl->allowRegainStamina = qfalse;
		SV_SendServerCommand(cl, "print \"Regain stamina deactivated\n\"");
		return;
	}
	else if (Cmd_Argv(1)[0] == '1') {
		cl->allowRegainStamina = qtrue;
		SV_SendServerCommand(cl, "print \"Regain stamina activated\n\"");
		return;
	}
	else {
		SV_SendServerCommand(cl, "print \"Unexpected parameter. Type \\help to list available commands\n\"");
		return;
	}
}
...
static void SV_UserAllowGoto_f(client_t *cl) {
	...
	if (Cmd_Argv(1)[0] == '0') {
		cl->allowGoto = qfalse;
		SV_SendServerCommand(cl, "print \"Your personal allow goto has been deactivated\n\"");
		return;
	}
	else if (Cmd_Argv(1)[0] == '1') {
		cl->allowGoto = qtrue;
		SV_SendServerCommand(cl, "print \"Your personal allow goto has been activated\n\"");
		return;
	}
	else {
		SV_SendServerCommand(cl, "print \"Unexpected parameter. Type \\help to list available commands\n\"");
...
static void SV_Goto_f(client_t *cl) {
	...
	myState->velocity[0] = 0;
	myState->velocity[1] = 0;
	myState->velocity[2] = 0;
	
	cl->lastGotoTime = svs.time;
	
	sprintf(targetMsg, sv_playerTeleportedMsg->string, cl->name);
	
	if (sv_gotoMsgBigtext->integer == 1) { 
		SV_SendServerCommand(cl, "cp \"%s\n\"", sv_gotoSuccessMsg->string);
		SV_SendServerCommand(targetCl, "cp \"%s\n\"", targetMsg);
	}
	else {
		SV_SendServerCommand(cl, "print \"%s\n\"", sv_gotoSuccessMsg->string);
		SV_SendServerCommand(targetCl, "print \"%s\n\"", targetMsg);
	}
...
static ucmd_t ucmds_floodControl[] = {
	...
	{"goto", SV_Goto_f},
	{"regainstamina", SV_UserAllowRegainStamina_f},
	{NULL, NULL}
};
...
void SV_ExecuteClientCommand( client_t *cl, const char *s, qboolean clientOK ) {
	...
	if (clientOK) {
		...
		if (!u->name && sv.state == SS_GAME) {
			...
			if (cl->muted && (!Q_stricmp("say", Cmd_Argv(0)) || !Q_stricmp("say_team", Cmd_Argv(0)) || !Q_stricmp("tell", Cmd_Argv(0)) || !Q_stricmp("ut_radio", Cmd_Argv(0)) || !Q_stricmp("callvote", Cmd_Argv(0)))) {
				SV_SendServerCommand(cl, "print \"^1You are currently muted and may not perform that action.\n\"");
				return;
			}
			
			qboolean goingToCallvoteCyclemap = qfalse;
			if (Cmd_Argc() >= 2 && !Q_stricmp("callvote", Cmd_Argv(0)) && !Q_stricmp("cyclemap", Cmd_Argv(1))) {
				int callvoteCyclemapWaitTime = sv_callvoteCyclemapWaitTime->integer;
				if (sv.lastCallvoteCyclemapTime > 0 && callvoteCyclemapWaitTime > 0) {
